...
namespace Utility.Math
{
    public static class MathUtility
    {
        ...
        public static float CCW(Vector2 p1, Vector2 p2, Vector2 p3)
        {
            ...
            else if (FloatZero(ret)) return 0; //일직선상
            else return -1; //시계 방향
        }
        ...
        /// <summary>
        /// 무작위로 놓인 점을 최대한 자연스러운 SimplePolygon처럼 보이게 연결해주는 함수입니다.
        /// </summary>
        ...
        public static List<Vector2> MakeSimplePolygon(List<Vector2> pts)
        {
            ...
            for (int i = 0; i < pts.Count; i++)
            {
                ...
                if (CCW(pts[leftId], pts[rightId], pts[i]) >= 0) LP.Add(pts[i]);
                else RP.Add(pts[i]);
            }
